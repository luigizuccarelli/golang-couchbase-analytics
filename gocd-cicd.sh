...
PROJECT="trackmate-couchbase-analytics"
jobname="kaniko-trackmate-couchbase-analytics"
deploymentconfig="trackmate-couchbase-analytics"
namespace="trackmate"
...
then
   ...
   fs=$(stat --printf='%s\n' output.json)
   result="\"PENDING\""
   ${SONARQUBE_SCANNER_PATH}bin/sonar-scanner -Dsonar.projectKey=${PROJECT} ${SONAR_LOGIN} -Dsonar.go.coverage.reportPaths=tests/results/cover.out -Dsonar.exclusions=vendor/**,cmd/**,pkg/connectors/**,pkg/schema/**,pkg/handlers/*_test.go,pkg/validator/*_test.go,swaggerui/**,tests/**,charts/**,*.json,*.txt,*.yml,*.xml,*.sh,Dockerfile -Dsonar.issuesReport.json.enable=true -Dsonar.report.export.path=sonar-report.json -Dsonar.issuesReport.console.enable=true | tee response.txt
